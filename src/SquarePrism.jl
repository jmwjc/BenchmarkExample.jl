...
function generateMsh(filepath::String; lc = 1.0, transfinite = -1, order = 1, quad = false)
    ...
    if transfinite > 0
        ...
        gmsh.model.mesh.setTransfiniteCurve(L5, 6*transfinite)
        gmsh.model.mesh.setTransfiniteCurve(L6, 6*transfinite)
        gmsh.model.mesh.setTransfiniteCurve(L7, 6*transfinite)
        gmsh.model.mesh.setTransfiniteCurve(L8, 6*transfinite)
...
@inline function generateGeo(lc = 1.0)
    ...
    gmsh.model.geo.addPoint(0.0,  0.0, H, lc, 5)
    gmsh.model.geo.addPoint(  𝐿,  0.0, H, lc, 6)
    gmsh.model.geo.addPoint(  𝐿,    𝐿, H, lc, 7)
    gmsh.model.geo.addPoint(0.0,    𝐿, H, lc, 8)
